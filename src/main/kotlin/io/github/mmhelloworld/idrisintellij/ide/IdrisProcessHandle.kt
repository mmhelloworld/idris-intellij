...
import io.github.mmhelloworld.idrisintellij.protocol.Reply
import io.github.mmhelloworld.idrisintellij.protocol.RuntimeProbe
...
class IdrisProcessHandle private constructor(
    ...
    companion object {
        ...
        fun start(executable: String, extraArgs: List<String>, root: Path, onClosed: (Throwable?) -> Unit): IdrisProcessHandle {
            val backendVersion = JvmBackend.detectVersion(executable)
            if (backendVersion != null && backendVersion < JvmBackend.MINIMUM) {
                throw IllegalStateException(
                    "idris2-jvm $backendVersion ($executable) is older than the minimum supported " +
                        "version ${JvmBackend.MINIMUM}. Install idris2-jvm ${JvmBackend.MINIMUM} or newer " +
                        "(GitHub releases or Maven Central) and update the path in " +
                        "Settings | Languages & Frameworks | Idris 2.")
            }
            ...
            when (connection.probeRuntime()) {
                ...
                RuntimeProbe.LEGACY_RUNTIME -> {
                    ...
                    process.destroyForcibly()
                    fail(
                        "This idris2 build ($executable) has the legacy JVM runtime stdio bug: " +
                            "ide-mode replies stall until more input arrives, so every request would time out. " +
                            "Point the plugin at idris2-jvm ${JvmBackend.MINIMUM} or newer " +
                            "in Settings | Languages & Frameworks | Idris 2.")
                }
