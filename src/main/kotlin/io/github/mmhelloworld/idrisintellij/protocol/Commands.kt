...
object IdeCommands {
    ...
    /**
     * Full callable-member catalog (signatures only) for one Java class, resolved against
     * [classpath] (a path-separator-separated list of jars/dirs; "" = JDK only). Drives Java-FFI
     * member completion: it surfaces the WHOLE surface of a class, including members not yet
     * generated by `--jvm-ffi-import`. Reply payload is the catalog text (see ResultDecoder.parseCatalog).
     */
    fun jvmFfiList(classpath: String, internalClassName: String): SExp =
        list(sym("jvm-ffi-list"), str(classpath), str(internalClassName))
}
...
/** One callable member from a `(:jvm-ffi-list ...)` catalog: the (overload-shared) call name and its rendered Idris signature. */
data class CatalogMember(val name: String, val signature: String)
...
object ResultDecoder {
    ...
    /** Member name : signature line; tolerates 2-space (bare) and 4-space (overload sub-namespace) indents. */
    private val CATALOG_MEMBER = Regex("""^\s+([A-Za-z_][A-Za-z0-9_']*)\s+:\s+(.+)$""")

    /**
     * Parse the `--jvm-ffi-list` catalog TEXT (the verbatim reply string) into members. Lines look like
     *   `  <name> : <signature>`     (canonical / non-overloaded, 2-space indent)
     *   `    <name> : <signature>`   (an overload inside a `namespace <Tag>` block, 4-space indent)
     * interleaved with `-- <class>  (import ...)` and `namespace <X>` lines (neither has ` : `, so both
     * are skipped). Overloads share a call name, so they collapse to ONE entry (first/canonical signature
     * kept) — the user types the name and Idris resolves the overload by argument type.
     */
    fun parseCatalog(text: String): List<CatalogMember> {
        val seen = LinkedHashSet<String>()
        val members = ArrayList<CatalogMember>()
        for (line in text.lineSequence()) {
            val match = CATALOG_MEMBER.find(line) ?: continue
            val name = match.groupValues[1]
            if (seen.add(name)) members.add(CatalogMember(name, match.groupValues[2].trim()))
        }
        return members
    }
